...
FlowNodes::FlowRoute.draw do
  ...
  node :llm_classifier do
    route [:content_analysis, :summarization], to: :content_processor >> :formatter
  ...
  node :api_router do 
    route [:success, :partial], to: :response_builder
    route [:error, :timeout], to: :error_response_builder
